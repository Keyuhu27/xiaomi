...
- 🔍 **关键词竞对**：三个视图——单日排名 / 一段时间的搜索趋势 / 按品牌拆开的排名汇总
- 🏆 **商品排名竞对**：筛选表格
- 📤 **数据导入**：上传京东商智相关的 Excel 文件

## 数据库里有什么（重要，务必先看）

这个数据库**不是一张"销量表"**，而是对应京东商智 5 类完全不同的报表，分别建了 5 张表
（已经用真实导出文件验证过表结构、字段含义、去重逻辑）：

| 表 | 内容 | 粒度 | 说明 |
|---|---|---|---|
| **sku_daily** | 全量商品每日库存/出库快照 | 日期 × SKU × 区域(RDC) | 官方结算后的**"昨天最终数据"**，回答"昨天/某天某型号卖了多少"的核心表。`sales_qty` = 昨日出库件数 |
| **sku_realtime** | 单品实时榜单 | 日期(=统计窗口所属自然日) × SKU | **"今天从0点到现在"还在累积中的数据**，不是最终数字；查"实时/目前为止卖了多少"用这张表。没有日期列，统计窗口靠解析文件名得到（见下面单独一节） |
| **funnel_daily** | 重点新品（Turbo 系列等）全链路流量转化 | 日期 × 产品系列 × 渠道 | 有 PV/UV/加购/下单/成交全链路数据，`sales_qty`=成交子单量，`sales_amount`=成交金额 |
| **keyword_brand_daily** | 行业关键词/品牌词条竞对监控 | 日 × 品牌 × 关键词 | 覆盖全行业，数值多为脱敏区间（如"10万~25万"），`_est` 字段是区间中点估算值，不是精确数 |
| **keyword_sku_rank_daily** | 关键词/型号下商品排名竞对监控 | 日 × 型号 × SKU | 含自家和竞品，数值同样多为脱敏区间估算 |

> ⚠️ 这两张表最初起名带"weekly"，是看源数据有个"周"字段就想当然认为是周度数据；
> 后来对着真实数据核对才发现"日期"字段每天都有一行、时间跨度以年计，其实是
> **每日数据**——已经改名成 `keyword_brand_daily` / `keyword_sku_rank_daily`，
> 表里的 `week` 字段只是原表附带的周标签，不代表按周聚合。
...
### ⚠️ sku_realtime 依赖文件名，别改文件名

"实时榜单_SKU"这份报表本身**没有日期列**，统计窗口（从几点到几点）只写在文件名里，
形如 `..._20260901_00_00_00_20260901_15_35_25.xlsx`（前一段是窗口起点，通常是当天
0 点；后一段是导出的那个时刻）。导入脚本靠正则从文件名解析这两个时间点，写进
`window_start` / `snapshot_at` / `date` 字段——**如果把文件重命名成别的格式，这张表
会直接跳过导入并报错提示**，不会导错数据。上传时保持京东商智导出的原始文件名即可。
...
2. 导入数据，两种方式任选一种（部署到 Railway 后只能用方式 A，因为没有本地文件系统）：
   ...
   不管哪种方式，脚本都会打开文件、逐个 sheet 自动识别是上面 5 类报表的哪一种（认不出的
   ...
   | sku_realtime | 按 `date`（同一天重新导出，用最新快照整体替换） |
   | funnel_daily | 按 `(series_code, date)`（同一产品系列同一天的数据整体替换，不影响其它日期） |
   | keyword_brand_daily / keyword_sku_rank_daily | 按 `date`（同一天的数据整体替换） |
...
- **异常检测预计算**：现在是每次提问时现算，如果以后数据量变大、想要更快，可以加一个每晚跑一次的
  定时任务，把统计结果预先存好。
- **keyword_brand_daily / keyword_sku_rank_daily 的估算值**：目前用区间中点做估算
