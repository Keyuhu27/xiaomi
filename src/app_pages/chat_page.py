...
数据库里有 5 张表，对应 5 类不同的京东商智报表，字段和口径的详细说明见 run_sql 工具描述，
关键区别：
- sku_daily：全量商品每日销量快照，是官方结算后的"昨天最终数据"，回答"昨天/某天卖了多少"优先查这张表。
- sku_realtime：单品实时榜单，是"今天从0点到现在"还在累积中的数据，不是最终数字。查"现在/实时/
  目前为止卖了多少"用这张表，跟 sku_daily 的口径不一样，不要混用或相加。
