...
TOOLS = [
    {
        ...
        "description": (
            ...
            "数据库里有 5 张表，对应京东商智 5 类不同的报表（口径不同，不要混用）：\n"
            "\n"
            "1) sku_daily —— 全量商品每日库存/销量快照，覆盖所有 SKU，是官方结算后的"
            "'昨天最终数据'，回答'今天/某天某型号卖了多少'优先查这张表。"
            ...
            "\n"
            "2) sku_realtime —— 单品实时榜单，是'今天从0点到导出那一刻'还在累积中的数据，"
            "不是当天最终数字（当天数据要等 sku_daily 第二天才有）。查'现在/实时/目前为止卖了"
            "多少'这类问题用这张表，查'昨天/某天最终卖了多少'用 sku_daily，不要混用。"
            "字段：date(所属自然日), window_start(统计窗口起点，通常是当天0点), "
            "snapshot_at(导出快照的时刻), sku_id, product_name, "
            "pv/uv/paying_customers(成交人数)/conversion_rate(成交转化率)/"
            "sales_qty(成交商品件数)/order_qty(成交单量)/sales_amount(成交金额)/aov(成交客单价)/"
            "follow_customers(关注人数)/add_cart_customers/add_cart_qty/add_cart_rate/"
            "add_cart_qty_positive/add_cart_qty_negative/uv_value，每个指标还有 _cmp"
            "（对比时间段的值）和 _chg（涨跌幅，小数）两个附属字段。同一个 sku_id 在同一个 date"
            "只有一行（不用担心区域拆分重复计数）。\n"
            "\n"
            "3) funnel_daily —— 重点新品（如 Redmi Turbo 系列）的每日全链路流量转化数据，"
            ...
            "是否在首销后对应时间窗口内。\n"
            "\n"
            "4) keyword_brand_daily —— 行业关键词/品牌词条竞对监控，每日数据（不是周度，"
            "date 字段每天都有一行；week 字段只是原表附带的周标签，不代表按周聚合），覆盖全行业"
            "（不只是自家）。字段：date, spu, brand, keyword(关键词), rank(排名), "
            ...
            "\n"
            "5) keyword_sku_rank_daily —— 关键词/型号下的商品排名竞对监控（含自家和竞品"
            "SKU），每日数据（同上，不是周度）。字段：date, spu(型号分类), rank(排名), sku_id, "
            "product_info(商品全名), brand_name, 以及 click_users/click_count/sales_qty/"
            "sales_amount 的 _raw/_est 字段（同上，_est 是区间估算值）。\n"
            "\n"
